<!-- Carousel -->
<div id="carousel1" class=" container carousel slide carousel-fade hoverable">

    <!-- Indicators -->
    <ol class="carousel-indicators">
        <li data-target="#carousel1" data-slide-to="0" class="active"></li>
        <li data-target="#carousel1" data-slide-to="1"></li>
        <li data-target="#carousel1" data-slide-to="2"></li>
        <li data-target="#carousel1" data-slide-to="3"></li>
    </ol>

    <!-- Wrapper for slides -->
    <div class="carousel-inner z-depth-2" role="listbox">

        <!-- First slide -->
        <div class="item active" id="tf-home">
            <div class="view overlay hm-blue-slight">
                <a><img src="/store/src/public/images/slider/main_4.jpg" alt="Welcome To Our Humble Watches Store">
                    <div class="mask waves-effect waves-light"></div>
                </a>
                <div class="carousel-caption hidden-xs">
                    <div class="animated fadeInDown">
                        <h1><strong><span class="color">Shine On</span></strong></h1>
                        <p class="lead ">Shop your favorite Jewelery,various brands and more!</p>
                    </div>
                </div>
            </div>
        </div>


        <div class="item" id="tf-home">
            <div class="view overlay hm-blue-slight">
                <a><img src="/store/src/public/images/slider/main_1.jpg" alt="Watch">
                    <div class="mask waves-effect waves-light"></div>
                </a>
                <div class="carousel-caption hidden-xs">
                    <div class="animated fadeInDown">
                    </div>
                </div>
            </div>
        </div>
        <!-- /.item -->

        <!-- Second slide -->
        <div class="item" id="tf-home">
            <div class="view overlay hm-blue-slight">
                <a><img src="/store/src/public/images/slider/main_2.jpg" alt="Watch">
                    <div class="mask waves-effect waves-light"></div>
                </a>
                <div class="carousel-caption hidden-xs">
                    <div class="animated fadeInDown">
                    </div>
                </div>
            </div>
        </div>
        <!-- /.item -->

        <!-- Third slide -->
        <div class="item" id="tf-home">
            <div class="view overlay hm-blue-slight">
                <a><img src="/store/src/public/images/slider/main_3.jpg" alt="Watch">
                    <div class="mask waves-effect waves-light"></div>
                </a>
                <div class="carousel-caption hidden-xs">
                    <div class="animated fadeInDown">
                    </div>
                </div>
            </div>
        </div>
        <!-- /.item -->
    </div>
    <!-- /.carousel-inner -->

    <!-- Controls -->
    <a class="left carousel-control new-control" href="#carousel1" role="button" data-slide="prev">
        <span class="fa fa fa-angle-left waves-effect waves-light"></span>
        <span class="sr-only">Previous</span>
    </a>
    <a class="right carousel-control new-control" href="#carousel1" role="button" data-slide="next">
        <span class="fa fa fa-angle-right waves-effect waves-light"></span>
        <span class="sr-only">Previous</span>
    </a>

</div>
<!-- /.carousel -->


